param(
    [Parameter(Mandatory = $true)]
    [string]$Version
)

$repo = "llm-supermarket/cli-deepseekv4-flash-go"
$exeName = "cli-deepseekv4-flash-go-windows-amd64.exe"
$exePath = Join-Path $PSScriptRoot "artifacts" $exeName

if (-not (Test-Path $exePath)) {
    throw "Unable to locate $exeName at $exePath"
}

$hash = (Get-FileHash -Path $exePath -Algorithm SHA256).Hash.ToLower()

Write-Host "Hash: $hash"

$url = "https://github.com/$repo/releases/download/v$Version/$exeName"

$manifestPath = Join-Path $PSScriptRoot "cli-deepseekv4-flash-go.json"

$manifest = Get-Content -Path $manifestPath -Raw | ConvertFrom-Json

$manifest.version = $Version
$manifest.architecture."64bit".url = $url
$manifest.architecture."64bit".hash = $hash

$manifest | ConvertTo-Json -Depth 10 | Set-Content -Path $manifestPath -NoNewline

Write-Host "Updated cli-deepseekv4-flash-go.json to v$Version"
